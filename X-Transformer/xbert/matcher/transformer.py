...
class TransformerMatcher(object):
    ...
    def train(self, args, trn_features, eval_features=None, C_eval=None):
        ...
        print("  Num examples = %d", len(trn_features))
        ...
        pirint("  Total optimization steps = %d", t_total)
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                        ...
                        print("| [{:4d}/{:4d}][{:6d}/{:6d}] | {:4d}/{:4d} batches | ms/batch {:5.4f} | train_loss {:6e} | lr {:.6e}".format(
                                int(epoch),
                                int(args.num_train_epochs),
                                int(global_step),
                                int(t_total),
                    ...
                    if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                        ...
                        print("| epoch {:3d} step {:6d} evaluation | time: {:5.4f}s | eval_loss {:e}".format(
                                epoch, global_step, total_run_time, eval_loss.item())
                        logger.info("| matcher_eval_prec {}".format(" ".join("{:4.2f}".format(100 * v) for v in eval_metrics.prec)))
                        print("| matcher_eval_prec {}".format(" ".join("{:4.2f}".format(100 * v) for v in eval_metrics.prec)))
                        logger.info("| matcher_eval_recl {}".format(" ".join("{:4.2f}".format(100 * v) for v in eval_metrics.recall)))
                        print("| matcher_eval_recl {}".format(" ".join("{:4.2f}".format(100 * v) for v in eval_metrics.recall)))
                        ...
                        if avg_matcher_prec > best_matcher_prec and epoch > 0:
                            ...
                            print("| **** saving model at global_step {} ****".format(global_step))
...
def main():
    ...
    if args.do_eval:
        ...
        print('saving C_trn_pred.npz and trn_embedding.npy...')
        ...
        print('saving C_eval_pred.npz and tst_embedding.npy...')
